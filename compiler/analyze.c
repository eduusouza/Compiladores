	...
	switch (no->nodekind){
		case Declaracao_NodeK:
			...
			case Call_StmtK:	
				...
				if ((strcmp(CharVerificaOcorrenciaNoMesmoEscopo(no->attr.name, "Global", "tipoDeclaracao"), " ") == 0) && 
				...
				strcmp(no->attr.name, "temporizador") != 0 &&
				...
				if(VerificaEquivalenciaParametrosDaFuncao(no->attr.name, contaQtdArgumentos) != 1  && 
				((strcmp(no->attr.name, "input") != 0 && 
				strcmp(no->attr.name, "output") != 0 && 
				strcmp(no->attr.name, "LOADInst") != 0 &&
				strcmp(no->attr.name, "saveContext") != 0 &&
